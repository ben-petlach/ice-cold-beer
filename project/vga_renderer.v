...
// active region: H offset 160, V offset 35
wire [9:0] screen_x = h_cnt[9:0] - 10'd160;
wire [9:0] screen_y = v_cnt      - 10'd35;

wire [7:0] game_x = screen_x[9:2];  // 0-159
wire [6:0] game_y = screen_y[8:2];  // 0-119

// Bar interpolation: slope*(x-39)*205>>14
...
// Ball sprite (7×7, centered at draw_ball_x/y)
...
wire [6:0] draw_ball_y = ball_surface_y[6:0] - 7'd3; // 3 rows above bar surface

// 7×7 sprite; bit 6 = left pixel
...
// highlight: (col=4,row=1) and (col=5,row=2)
...
// Holes: 8×8 tiles, diagonal corner cut, OR-reduced signals
...
// Background ROM: 160×120 1-bit, LUT-based (no M9K pipeline), hex file required
...
// HUD (game_x 120-159): 3×5 number_driver digits + ball indicators

// ROUND digit (level+1) at (149,1)
...
// LVL digit (current_step) at (140,8)
...
// 5 HUD balls; filled when balls_remaining >= i+2
...
always @(*) begin
    ...
    if (!blank_n) begin
        ...
        if (game_x == 8'd39 || game_x == 8'd119) begin
            ...
            // ball: gray body, white highlight
            r_next = 1'b1; g_next = 1'b1; b_next = 1'b1;
            ...
            // HUD balls: gray filled or hollow ring
